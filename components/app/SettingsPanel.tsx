...
export default function SettingsPanel({ open, appVersion, sessions, onClose, onHistoryDeleted }: SettingsPanelProps) {
  ...
  const [microphoneAvailability, setMicrophoneAvailability] = useState<MicrophonePermissionStatus | null>(null);
  ...
  const microphoneGateComplete =
    microphoneChecked && !microphonePromptOpen && (microphonePromptDismissed || microphoneAvailability?.permissionGranted);

  useEffect(() => {
    let cancelled = false;

    async function checkFirstOpenMicrophonePrompt() {
      if (typeof window === "undefined") return;
      const status = await checkMicrophonePermission().catch(() => null);
      if (cancelled) return;

      setMicrophoneAvailability(status);
      setMicrophoneChecked(true);
      setMicrophonePromptOpen(shouldShowFirstOpenMicrophonePrompt(status, false));
      if (!status || status.permissionGranted || status.state === "unsupported") {
        setMicrophonePromptDismissed(true);
      }
    }

    void checkFirstOpenMicrophonePrompt();
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (!microphoneGateComplete) return;
  ...
  useEffect(() => {
    ...
    async function checkNotificationPrompt() {
      if (typeof window === "undefined") return;
      if (!microphoneGateComplete || walkthroughOpen) return;
  ...
  }, [microphoneGateComplete, walkthroughOpen]);

  if (!open && !walkthroughOpen && !notificationPromptOpen && !microphonePromptOpen) return null;
  ...
  async function refreshMicrophonePermission() {
    setMicrophoneStatus("Checking microphone access...");
    const status = await checkMicrophonePermission();
    setMicrophoneAvailability(status);

    if (status.permissionGranted) {
      setMicrophoneStatus("Microphone is enabled on this device.");
      return;
    }

    if (status.state === "unsupported") {
      setMicrophoneStatus("Microphone recording is not supported in this app.");
      return;
    }

    setMicrophoneStatus("Microphone access is needed to record a hum.");
  }

  async function enableMicrophone() {
    setMicrophoneStatus("Checking microphone access...");
    const status = await requestMicrophonePermissionAfterUserAction();
    setMicrophoneAvailability(status);
  ...
  function dismissMicrophonePrompt() {
    setMicrophonePromptDismissed(true);
    setMicrophonePromptOpen(false);
  }
  ...
  return (
    <>
              ...
              {activeSection === "data" ? (
                <SettingsCard title="Local data controls">
                  <MicrophonePermissionCard
                    ...
                    onEnable={microphoneAvailability?.state === "granted" || microphoneAvailability?.state === "denied"
      ...
      {microphonePromptOpen ? (
        <FirstOpenMicrophonePrompt
          status={microphoneStatus}
          onEnable={enableMicrophone}
          onDismiss={dismissMicrophonePrompt}
        />
      ) : null}
...
export function MicrophonePermissionCard({
  ...
  const copy = getMicrophoneSettingsCopy(status);

  return (
    <section className="notification-opt-in-card microphone-permission-card" aria-labelledby="microphone-permission-title">
      <div>
        <h4 id="microphone-permission-title">{copy.title}</h4>
        <p>{copy.body}</p>
        {message ? <p className="settings-status-note">{message}</p> : null}
      </div>
      {copy.buttonLabel ? (
        <button type="button" className="settings-primary-button" disabled={copy.buttonDisabled} onClick={onEnable}>
          {copy.buttonLabel}
...
export function FirstOpenMicrophonePrompt({
  status,
  onEnable,
  onDismiss,
}: {
  ...
  return (
    <div className="notification-prompt-overlay microphone-prompt-overlay" role="presentation">
      <section className="notification-prompt-card microphone-prompt-card" role="dialog" aria-modal="true" aria-labelledby="microphone-prompt-title">
        <p className="settings-kicker">First use</p>
        <h2 id="microphone-prompt-title">Microphone access is needed</h2>
        <p>
          Hum works by listening to a short hum. Audio stays local while Hum reads the shape of your voice. Without
          microphone access, you cannot record a hum.
        </p>
        {status ? <p className="settings-status-note">{status}</p> : null}
